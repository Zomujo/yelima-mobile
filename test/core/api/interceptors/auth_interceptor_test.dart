...
class MockSessionLifecycleService extends Mock
...
void main() {
  ...
  group('AuthInterceptor', () {
    test('onRequest adds Bearer token when token is available', () async {
      when(() => mockTokenManager.getValidAuthToken())
      ...
      verify(() => handler.next(any(
          that: isA<RequestOptions>().having((o) => o.headers['Authorization'],
              'Authorization', 'Bearer fake_token')))).called(1);
    });

    test('onRequest does not add token when getValidAuthToken returns null',
        () async {
      when(() => mockTokenManager.getValidAuthToken())
